...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\RETURN $\mi{PASS}$
		\ELSIF{$\exists i \in Al$,$\forall init \in I_i$.$Vl_i(\mi{init},\mi{iter}) > Ul_i(\mi{init},\mi{iter}) \wedge Vl_i(\mi{init},\mi{iter}) - Ul_i(\mi{init},\mi{iter}) > \mi{max}_{\mi{Aerror}}$}
		\RETURN $\mi{FAIL}$
		\ENDIF
		\ENDIF
		\IF{perfect}
		...
		\RETURN $\mi{PASS}$
		\ELSIF{$\exists i \in Al$, $\forall \mi{init} \in I$. $V_i(\mi{init},\mi{iter}) > U_i(\mi{init},\mi{iter})
